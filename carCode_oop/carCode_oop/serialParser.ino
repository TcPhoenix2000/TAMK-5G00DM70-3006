...
void serialParser::parseSerialData() {
  //serialEvent();
  ...
  if (line.startsWith("Move:")) {
    ...
    int initDist = Mylidar.getDistance();

    initDist -= moveValue;

    if (program == 1) { MyMove.moveDist(moveValue, initDist); }
    if (program == 2) { MyMove.executeMovementSequence(moveValue); }
  }
  ...
  if (line.startsWith("Turn:")) {
    ...
    MyMove.TurnAngle(turnValue, tolerance);
    Serial.println("done");
  }
}

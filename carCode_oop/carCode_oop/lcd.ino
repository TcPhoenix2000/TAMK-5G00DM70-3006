...
void Lcd::lcddisp() {
  float byteAngle = Mycompass.Compass();
...
void Lcd::printDist() {  // week 3 exersize 1 part 1
  // Get the LIDAR distance measurement
  lcd.setCursor(0, 1);
  int distance = Mylidar.getDistance();  // Get the distance in cm
  lcd.print("Distance: ");
  lcd.print(distance);
  lcd.print(" cm");
...
void Lcd::printMoveDist() {
  // Print the movedist
  lcd.setCursor(0, 1);
  lcd.print("move dist: ");
  lcd.setCursor(11, 1);
  lcd.print(Myjoystick.joystickPotentio());
}
void Lcd::printPress() {
  lcd.setCursor(0, 2);
